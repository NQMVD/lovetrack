...
local scroll_indicator = { x = 0, y = 0, timer = 0 }
...
function love.load()
  ...
  gesture_detector = GestureDetector.new("lovetrack")
  gesture_detector:start()

  -- Set up gesture callbacks
  gesture_detector:onScroll(function(vx, vy, accumulated_x, accumulated_y)
    -- Add to scroll history for visualization
    table.insert(scroll_history, { x = vx, y = vy, time = love.timer.getTime() })
    if #scroll_history > max_scroll_history then
      table.remove(scroll_history, 1)
    end

    -- Update scroll indicator
    scroll_indicator.x = vx * 100
    scroll_indicator.y = vy * 100
    scroll_indicator.timer = 1.0

    print(string.format("Scroll: vx=%.3f, vy=%.3f, acc_x=%.3f, acc_y=%.3f", vx, vy, accumulated_x, accumulated_y))
  end)
  ...
  print("- Use 1 finger to pan around")
  print("- Use 2 fingers to scroll or zoom (pinch/expand)")
end
...
function love.update(dt)
  ...
  -- Clean up old scroll history
  local current_time = love.timer.getTime()
  for i = #scroll_history, 1, -1 do
    if current_time - scroll_history[i].time > 2.0 then
      table.remove(scroll_history, i)
    end
  end
...
function love.draw()
  ...
  if scroll_indicator.timer > 0 then
    ...
    love.graphics.line(
      center_x, center_y,
      center_x + scroll_indicator.x, center_y + scroll_indicator.y
    )
    love.graphics.print("SCROLL", center_x + scroll_indicator.x + 10, center_y + scroll_indicator.y)
  end
  ...
  if gesture_detector:isScrolling() then
    local vx, vy = gesture_detector:getScrollVelocity()
    love.graphics.setColor(0, 1, 1)
    love.graphics.print(string.format("SCROLLING: vx=%.3f, vy=%.3f", vx, vy), 10, y_offset)
    y_offset = y_offset + 20
  end

  if gesture_detector:isPanning() then
    local x, y, dx, dy = gesture_detector:getPanState()
    love.graphics.setColor(1, 0.5, 0)
    love.graphics.print(string.format("PANNING: pos(%.3f, %.3f), delta(%.3f, %.3f)", x, y, dx, dy), 10, y_offset)
    y_offset = y_offset + 20
  end

  if gesture_detector:isZooming() then
    local cx, cy, scale = gesture_detector:getZoomState()
    love.graphics.setColor(1, 0, 1)
    love.graphics.print(string.format("ZOOMING: center(%.3f, %.3f), scale=%.3f", cx, cy, scale), 10, y_offset)
    y_offset = y_offset + 20
  end
  ...
  love.graphics.print("• 1 finger: Pan around", 10, love.graphics.getHeight() - 60)
  love.graphics.print("• 2 fingers: Scroll or Zoom (pinch/expand)", 10, love.graphics.getHeight() - 40)
  love.graphics.print("• ESC: Quit", 10, love.graphics.getHeight() - 20)
end
